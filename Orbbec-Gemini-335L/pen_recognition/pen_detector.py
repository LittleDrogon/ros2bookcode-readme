"""笔检测：微调后的 YOLOv8n（ONNX）+ OpenCV 预处理/NMS/画框。
...
DEFAULT_MODEL = PACKAGE_DIR / "models" / "yolov8n_pen.onnx"
...
# 只保留青色笔：OpenCV HSV 中青色大概在 85~105
CYAN_HUE_MIN = 85
CYAN_HUE_MAX = 105
CYAN_MIN_RATIO = 0.18
ALLOWED_COLORS = ("青色",)
...
class YoloV8PenDetector:
    def __init__(
        ...
        if not self.model_path.is_file():
            raise FileNotFoundError(f"找不到笔检测模型: {self.model_path}")
    ...
    def detect(self, image_bgr: np.ndarray) -> List[PenDetection]:
        if image_bgr is None or image_bgr.size == 0:
            return []
        tensor, meta = self._letterbox(image_bgr, self.imgsz)
        outputs = self.session.run(None, {self.input_name: tensor})
        boxes, scores, class_ids = self._decode(outputs[0], meta)
        if boxes.size == 0:
            return []
        ...
        found: List[PenDetection] = []
        for idx in np.array(keep).reshape(-1):
            x1, y1, x2, y2 = boxes[idx]
            conf = float(scores[idx])
            cls_id = int(class_ids[idx])
            type_key, type_name = CLASS_NAMES.get(cls_id, ("pen", "笔"))
            x1i, y1i, x2i, y2i = int(x1), int(y1), int(x2), int(y2)
            w = max(1, x2i - x1i)
            h = max(1, y2i - y1i)
            box = np.array([[x1i, y1i], [x2i, y1i], [x2i, y2i], [x1i, y2i]], dtype=np.float32)
            crop = image_bgr[y1i:y2i, x1i:x2i]
            if not _is_cyan_pen(crop):
                continue
            found.append(
                PenDetection(
                    type_key=type_key,
    ...
    def _decode(self, output: np.ndarray, meta: dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        # 自定义 4 类: (1, 4+4, N) = (1, 8, 8400)
        ...
        if not np.any(mask):
            ...
            return np.zeros((0, 4), dtype=np.float32), empty, empty.astype(np.int32)

        boxes = self._xywh_to_xyxy(boxes_xywh[mask])
        boxes = self._unmap(boxes, meta)
        return boxes.astype(np.float32), scores[mask].astype(np.float32), class_ids[mask].astype(np.int32)
...
def draw_detections(image_bgr: np.ndarray, detections: Sequence[PenDetection]) -> np.ndarray:
    canvas = image_bgr.copy()
    for det in detections:
        color = BOX_COLORS.get(det.type_key, (0, 255, 0))
        x, y, w, h = det.bbox
    ...
    for det in detections:
        ...
        draw.text((x, y), label, font=font, fill=(color[2], color[1], color[0]))
...
def _is_cyan_pen(crop: np.ndarray) -> bool:
    """框内是否有足够比例的青色像素（取中心区域，减少桌面干扰）。"""
    ...
    return float(np.mean(cyan)) >= CYAN_MIN_RATIO
