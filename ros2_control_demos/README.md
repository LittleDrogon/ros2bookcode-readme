...
**Note:** There is **no** apt metapackage named `ros-humble-ros2-control-demos` (apt will report that the package cannot be located). Build from this source repository on the **`humble`** branch instead:
...
* `bringup/config/EXAMPLE_NAME_controllers.yaml` - parameters with controllers' setup for the example.
* `description/EXAMPLE_NAME.ros2_control.xacro` - XACRO file with `ros2_control`-URDF-tag with hardware setup and parameters.
* `description/EXAMPLE_NAME.urdf.xacro` - the main description for the example used to generate URDF on the fly that is published on the `/robot_description` topic.
...
**Lyrical** | [`master`](https://github.com/ros-controls/ros2_control_demos/tree/master) | see above | [Documentation](https://control.ros.org/lyrical/doc/ros2_control_demos/doc/index.html)
